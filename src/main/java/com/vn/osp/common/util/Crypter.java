 ...
 * the network stream. Contains static methods to encrypt and compare passwords
 * with Unix encrypted passwords.<BR>
 *
...
public class Crypter {
    ...
    private static char[] saltChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789./"
            .toCharArray();
    ...
    private static int cov_2char[] = {0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34,
     ...
     * </P>
     *
     * @param enteredPassword The password to be encrypted.
     * @return A string consisting of the 2-character salt followed by the
     * encrypted password.
     */
    public static String crypt(String enteredPassword) {
        java.util.Random randomGenerator = new java.util.Random();
        int numSaltChars = saltChars.length;
        String salt;
        String enteredPassword1, enteredPassword2;
        String encryptedPassword1, encryptedPassword2, encryptedPassword;
        int pwdlength = enteredPassword.length();
        salt = (new StringBuffer()).append(
                saltChars[Math.abs(randomGenerator.nextInt()) % numSaltChars])
                .append(
                        saltChars[Math.abs(randomGenerator.nextInt())
                                % numSaltChars]).toString();
        if (pwdlength < 9) {
            enteredPassword2 = "";
            encryptedPassword1 = crypt(salt, enteredPassword);
            encryptedPassword2 = crypt(salt, enteredPassword2);
            encryptedPassword = "A" + encryptedPassword1
                    + encryptedPassword2.substring(2, 13);

        } else {

            enteredPassword2 = enteredPassword.substring(8);
            enteredPassword1 = enteredPassword.substring(0, 8);
            encryptedPassword1 = crypt(salt, enteredPassword1);
            encryptedPassword2 = crypt(salt, enteredPassword2);
            encryptedPassword = "A" + encryptedPassword1
                    + encryptedPassword2.substring(2, 13);
        }
     ...
     * </P>
     *
     * @param encryptedPassword The <I>encryptedPassword</I>. The two characters at position
     *                          2 and 3 are assumed to be the salt. This string would be the
     ...
     */
    public static boolean matches(String encryptedPassword,
                                  String enteredPassword) {

        String newencryptedPassword;
        String newencryptedPassword1;
        String newencryptedPassword2;
        String newenteredPassword1;
        String newenteredPassword2;
        String salt = encryptedPassword.substring(1, 3);

        if (enteredPassword.length() < 9) {
            newenteredPassword2 = "";
            newencryptedPassword1 = crypt(salt, enteredPassword);
            newencryptedPassword2 = crypt(salt, newenteredPassword2);
            newencryptedPassword = "A" + newencryptedPassword1
                    + newencryptedPassword2.substring(2, 13);

        } else {
            newenteredPassword1 = enteredPassword.substring(0, 8);
            newenteredPassword2 = enteredPassword.substring(8);
            newencryptedPassword1 = crypt(salt, newenteredPassword1);
            newencryptedPassword2 = crypt(salt, newenteredPassword2);
            newencryptedPassword = "A" + newencryptedPassword1
                    + newencryptedPassword2.substring(2);
        }
