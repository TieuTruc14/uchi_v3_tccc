...
myApp.controller('contractEditController',['$scope','$http','$filter','$sce','$window','$q','$timeout','fileUpload',function ($scope,$http,$filter,$sce,$window,$q,$timeout,fileUpload) {
    ...
    // $scope.duongsu='<div ng-repeat="item in privys.privy track by $index" class="duongsuList"> <p style="margin-bottom: 2pt; font-family: Times New Roman; font-size: 14pt;" class=""><b style="font-family: Times New Roman; font-size: 14pt;" class="">Bên <span editable-text="item.action" class="inputcontract" contenteditable="true">{{item.action}}</span></b>(sau đây gọi là {{item.name}}):</p> <div ng-repeat="user in item.persons track by $index" class="personList"> <p style="font-family: Times New Roman; font-size: 14pt;" class="">Họ tên:&nbsp;<span href="#" editable-text="user.name" class="inputcontract" >{{user.name}}</span>&nbsp; <b style="font-family: Times New Roman; font-size: 14pt;" class=""> </b>, sinh năm: <span placeholder="" editable-text="user.birthday" class="inputcontract" >{{user.birthday}}</span> ; </p> <p style="font-family: Times New Roman; font-size: 14pt;" class="">Giấy CMND số:&nbsp;<span placeholder="" editable-text="user.passport" class="inputcontract" >{{user.passport}}</span> cấp ngày:&nbsp; <span placeholder="" editable-text="user.certification_date" class="inputcontract" >{{user.certification_date}}</span> tại: <span placeholder="" editable-text="user.certification_place" class="inputcontract">{{user.certification_place}}</span> ; </p>  <p style="font-family: Times New Roman; font-size: 14pt;" class="">Địa chỉ:&nbsp;<span placeholder="" editable-text="user.address" class="inputcontract" >{{user.address}}</span>; </p></div> </div>';
